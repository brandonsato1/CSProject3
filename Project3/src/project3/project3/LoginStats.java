...
public class LoginStats{
	public static void main(String[] args) {
		...
		while (inFile.hasNextLine()) {
			...
			try {
				Record current = new Record (terminal,terminal>0,name, new Date(time));
				list.add(  current  ); 
			}
		...
		while (!userValue.equalsIgnoreCase("quit")) {
			...
			if (userValue.substring(0,5).equalsIgnoreCase("first")) {
			...
			else if (userValue.equalsIgnoreCase("quit")) {
				...
				String username = userValue.substring(6,userValue.length()).trim();
				System.out.println("Username: "+username+ " Duration: " +list.getTotalTime(username));
			}
			else {
